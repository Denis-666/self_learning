...
Column **dump** configures file system for backup with _dump_ (program).
...
| `chown`   | `-R user:group filename` | Change owner (_User_ & _Group_) recursively       |
...
Change the _grub_ config `/etc/default/grub` to disalbe IPv6.
